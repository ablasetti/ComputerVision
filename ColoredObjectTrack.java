...
public class ColoredObjectTrack implements Runnable {
        ...
        try {
            ...
            while (true) {
                ...
                if (img != null) {
                    ...
                    canv3.showImage(getcontours(img));
            
                    canv4.showImage(DoPyra(img));
 
                    
                    IplImage detectThrs = getThresholdImage(img);
                    //cvSaveImage(++ii+"dsmthreshold.jpg", detectThrs);
                    canv2.showImage(detectThrs); 
    ...
    private IplImage getcontours(IplImage orgImg) {
    	IplImage ctimg = cvCreateImage(cvGetSize(orgImg), 8, 1);
    	
    	cvCvtColor(orgImg, ctimg, CV_BGR2GRAY);
    	//cvThreshold(ctimg, ctimg, 64, 255, CV_THRESH_BINARY);
    	//cvSmooth(ctimg, ctimg, CV_MEDIAN, 1);
    	IplImage destimg = IplImage.create(ctimg.width(), ctimg.height(), IPL_DEPTH_8U, 1);
    	cvCanny(ctimg,destimg,100,100,3);
    	
    	//cvSaveImage(++ii + "dsmthreshold.jpg", destimg);
    	
    	return destimg;
    }
    
 
    private IplImage DoPyra(IplImage orgImg) {
    	//IplImage ctimg = cvCreateImage(cvGetSize(orgImg), 8, 1);    	
    	IplImage pyr = cvCreateImage(cvSize(orgImg.width()/2, orgImg.height()/2), 8, 3);
    	cvPyrDown(orgImg,pyr,7);
